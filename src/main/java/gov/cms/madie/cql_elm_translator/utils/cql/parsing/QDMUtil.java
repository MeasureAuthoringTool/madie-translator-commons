...
public class QDMUtil {
  ...
  public static QDMContainer getQDMContainer() {
    ...
    nameToProfileInfoMap.forEach(
        (k, v) -> {
          ...
          // we still want to collect information about it, but we do not want attributes for the
          // base type, but
          // rather we want attributes for the current datatype so we will use it's name
          // (QDM.CareGoal) so that
          // we can look up information about the current datatypes attribute.

          // We only want to use base type information if it is a profile. For example,
          // QDM.PositiveEncounterPerformed
          // has a base type of QDM.EncounterPerformed. We can than use this base type to look up
          // the attributes for
          // that datatype.
          ...
          attributesForDataType.addAll(
              classInfo.getElement()); // adds data type specific attributes
          attributesForDataType.addAll(
              nameToClassInfoMap.get("QDM.QDMBaseType").getElement()); // add base type attributes
  ...
  private static void buildProfileAndClassInfoMaps(
      ModelInfo modelInfo,
      Map<String, TypeInfo> nameToProfileInfoMap,
  ...
  private static void collectAttributeTypeInformation(
      Map<String, List<String>> attributeToCQLTypeMap, String label, ClassInfoElement attribute) {
  ...
  private static void getAttributesFromChoiceTypeSpecifier(
      Map<String, List<String>> attributeToCQLTypeMap, ClassInfoElement attribute) {
